...
const MessagesAdvanced: React.FC = () => {
  // États
  ...
  const handleEditMessage = async (id: number) => {
    try {
      ...
      message.error('Impossible de charger le message');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendNow = async (id: number) => {
    try {
      ...
      if (result.success) {
        ...
        message.error(`Erreur lors de l'envoi: ${result.message}`);
      }
      loadData();
    } catch (error) {
      console.error('Erreur lors de l\'envoi du message:', error);
      message.error('Impossible d\'envoyer le message');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleProcessScheduled = async () => {
    try {
      ...
      message.success(`${result.success} messages traités avec succès`);
      loadData();
    } catch (error) {
      console.error('Erreur lors du traitement des messages programmés:', error);
      message.error('Impossible de traiter les messages programmés');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (currentMessage?.id) {
        ...
        message.success('Message mis à jour avec succès');
      } else {
      ...
      message.error('Impossible d\'enregistrer le message');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <DashboardLayout title="Messagerie avancée">
